...
```{r}
prior1 = c(
  prior(normal(1,0.5), coef="disp_index"),
  prior(normal(1,0.5), coef="age_myr")
)
```
...
model_creb$loo <- loo(model_creb)
null_creb1$loo <- loo(null_creb1)
loo_compare(null_creb1$loo,model_creb$loo)
```

Posterior predictive checks and conditional effects - CREB1
```{r}
pp_check(model_creb, nsamples =1000)
conditional_effects(model_creb)
```
...
plot_adcyap <- model_adcyap %>%
  spread_draws(b_Intercept, r_pop[pop,]) %>%
  mutate(condition_mean = b_Intercept + r_pop) %>%
  ggplot(aes(y = fct_reorder(pop,condition_mean), x = condition_mean)) +
  theme_minimal() + 
